...
describe('download api', () => {
  ...
  describe('Edge cases', () => {
    it('rejects empty URL in /add', async () => {
    ...
    it('rejects missing URL in /add', async () => {
      const response = await app.handle(
        new Request('http://localhost/api/download/add', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    it('rejects empty URL in /bulk', async () => {
      const response = await app.handle(
        new Request('http://localhost/api/download/bulk', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ urls: ['https://youtube.com/watch?v=a', ''] }),
        })
      );
      expect(response.status).toBe(422);
    });

    it('rejects empty array in /bulk', async () => {
      const json = (await requestJson(
        app,
        new Request('http://localhost/api/download/bulk', {
          method: 'POST',
    ...
    it('returns 404 for unknown route', async () => {
      const response = await app.handle(
        new Request('http://localhost/api/download/unknown')
      );
      expect(response.status).toBe(404);
    });

    it('handles pause for non-existent task', async () => {
      const json = (await requestJson(
        app,
        new Request('http://localhost/api/download/task/non-existent/pause', {
          method: 'POST',
        })
      )) as { success: boolean; data: null };
      expect(json.success).toBe(true);
      expect(json.data).toBeNull();
    });

    it('handles resume for non-existent task', async () => {
      const json = (await requestJson(
        app,
        new Request('http://localhost/api/download/task/non-existent/resume', {
          method: 'POST',
        })
      )) as { success: boolean; data: null };
      expect(json.success).toBe(true);
    ...
    it('handles delete for non-existent task', async () => {
      const json = (await requestJson(
        app,
        new Request('http://localhost/api/download/task/non-existent', {
          method: 'DELETE',
        })
      )) as { success: boolean; data: { removed: boolean } };
      expect(json.success).toBe(false);
      expect(json.data.removed).toBe(false);
    });
  });
});
